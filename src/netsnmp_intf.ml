...
module type S = sig
  ...
  module Oid : sig
    ...
    (** [of_string] - convert string to an [Oid.t], modules reference will be loaded if
        they are in the mib path *)
  ...
  module Connection : sig
    ...
    (** [connect] creates a connection to a peer based on the information passed in
        [Connection_info.t]. This and [close] should be called as a pair. *)
    val connect : Connection_info.t -> t IO.t

    (** [close] - close an existing connection *)
    val close : t -> unit IO.t

    (** [with_connection] - connect to the remote agent using the supplied credentials and
        then call the function [f] with the session. Closes the session in all cases
        before returning *)
    val with_connection : Connection_info.t -> f:(t -> 'a) -> 'a IO.t
  end

  (** [add_mib_paths] adds directories to search for mibs in. Note that this will be used
      for all subsequent mib searches. raises Failure on error *)
  val add_mib_paths : string list -> unit IO.t

  (** [get_s] - get the values of a list of symbolic oids. The function handles loading
      mib modules referenced by the supplied oids, eg passing 'SNMPv2-MIB::sysDescr.0'
      ensures SNMPv2-MIB is loaded. *)
